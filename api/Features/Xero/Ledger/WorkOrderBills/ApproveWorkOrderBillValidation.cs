...
/// <summary>The shape check: ids present, one coding per line, every share on an order and a code.</summary>
public sealed class ApproveWorkOrderBillValidation
{
    public ValidationOutcome Check(ApproveWorkOrderBill command)
    {
        ...
        foreach (var line in command.Lines ?? Array.Empty<WorkOrderBillLineCoding>())
        {
            ...
            if (line.Shares is null || line.Shares.Count == 0) errors.Add("Every line needs at least one share.");
            if (line.Shares?.Any(share => string.IsNullOrWhiteSpace(share.WorkOrderId)) == true)
                errors.Add("Every share needs a work order.");
            if (line.Shares?.Any(share => string.IsNullOrWhiteSpace(share.CostCenterCode)) == true)
